...
//#region src/util.ts
function l() {
	let e = document.getElementById("reactedge-runtime");
	if (!e) return;
	let t;
	try {
...
var u = /* @__PURE__ */ o(((e) => {
	...
	n(), t.exports = g();
})), v = /* @__PURE__ */ c(d(), 1), y = /* @__PURE__ */ c(_(), 1), b = null;
function x() {
	if (b) return b;
	let e = document.getElementById("reactedge-registry");
	if (!e) throw Error("Missing registry");
	return b = JSON.parse(e.textContent || "{}"), b;
}
function ee(e) {
	let t = e.tagName.toLowerCase().replace("-widget", "");
	return e.dataset.instance || t;
}
function S(e) {
	let t = x(), n = ee(e), r = e.tagName.toLowerCase().replace("-widget", ""), i = t[n];
	if (!i) throw Error(`No config for instance "${n}"`);
	return {
		type: r,
		entry: i
	};
...
var C = class {
	...
	async ensureFeature(e) {
		if (this.features.has(e)) return this.features.get(e);
		let t = await import(x()[e].src);
		return this.features.set(e, t), t;
	}
};
function te() {
	let e = window.ReactEdgeRuntime;
	if (!e) throw Error("ReactEdge runtime not available");
	return e;
}
async function ne(e) {
	let { type: t, entry: n } = S(e), r = await te().ensureFeature(t), i = l();
	r.mount(e, n?.contract ?? null, i);
}
function re(e) {
	return e.tagName.toLowerCase().replace("-widget", "");
}
function w() {
	document.querySelectorAll("[data-load]").forEach((e) => {
		if (!re(e)) return;
		let t = e.dataset.load || "lazy";
		if (t !== "ssr") {
			if (t === "critical") {
				try {
					ne(e);
...
function ie(e) {
	let t = new IntersectionObserver((n) => {
		n.forEach((n) => {
			n.isIntersecting && (ne(e), t.unobserve(e));
		});
	}, { rootMargin: "200px" });
	t.observe(e);
}
function ae(e) {
	document.readyState === "interactive" || document.readyState === "complete" ? queueMicrotask(e) : document.addEventListener("DOMContentLoaded", e);
}
function oe(e, t) {
	let n = t.match(/^on-scroll:(\d+)$/);
	if (!n) return;
	let r = Number(n[1]), i = () => {
		window.scrollY >= r && (ne(e), window.removeEventListener("scroll", i));
	};
...
document.readyState === "loading" ? document.addEventListener("DOMContentLoaded", se) : queueMicrotask(se);
var ce = new C();
window.ReactEdgeRuntime = ce, window.React = v.default, window.ReactDOM = y.default;
//#endregion
export { ne as mountWidget, w as scheduleWidgets };
